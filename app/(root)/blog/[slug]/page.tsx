...
export async function generateMetadata({ params }: BlogPostPageProps): Promise<Metadata> {
  ...
  if (!res.success || !res.data) {
    return {
      title: "নিবন্ধ পাওয়া যায়নি | Grihinir Bari",
  ...
  const pageTitle = post.seoTitle || `${post.title} | Grihinir Bari (গৃহিণীর বাড়ি) Journal`;
  const pageDescription = post.seoDescription || post.excerpt;

  return {
    title: pageTitle,
    description: pageDescription,
    keywords: post.seoKeywords?.length ? post.seoKeywords : post.tags || ["Grihinir Bari Journal", "খাঁটি সরষের তেল", "ঘানি গল্প"],
    alternates: {
    ...
    openGraph: {
      ...
      authors: [post.author || "Grihinir Bari Editorial"],
      tags: post.tags,
    },
    twitter: {
...
export default async function BlogPostDetailPage({ params }: BlogPostPageProps) {
  ...
  const articleJsonLd = {
    ...
    author: {
      "@type": "Person",
      name: post.author || "গৃহিণীর বাড়ি টিম",
    },
    publisher: {
      "@type": "Organization",
      name: "Grihinir Bari",
  ...
  return (
    <article className="min-h-screen bg-[#FDFBF7] text-[#271C16] pb-24">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="py-8 flex items-center justify-between border-b border-gray-200">
          ...
          <div className="flex gap-2">
            <a
              href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(post.title)}&url=${baseUrl}/blog/${post.slug}`}
              target="_blank"
              rel="noopener noreferrer"
              className="px-3 py-1.5 bg-[#FAF6EE] hover:bg-[#C88A2B] hover:text-white rounded-lg text-xs font-bold text-[#7A4117] transition flex items-center gap-1"
            >
              X / Twitter
            </a>
            <a
            ...
            <a
              href={`https://www.linkedin.com/shareArticle?mini=true&url=${baseUrl}/blog/${post.slug}&title=${encodeURIComponent(post.title)}`}
              target="_blank"
              rel="noopener noreferrer"
              className="px-3 py-1.5 bg-[#FAF6EE] hover:bg-[#C88A2B] hover:text-white rounded-lg text-xs font-bold text-[#7A4117] transition flex items-center gap-1"
            >
