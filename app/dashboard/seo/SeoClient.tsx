...
export default function SeoClient({ initialPages }: { initialPages: any[] }) {
  ...
  const domain = process.env.NEXT_PUBLIC_SERVER_URL || "https://grihinirbari.com";
      ...
      <div className="flex border-b border-gray-200 bg-white px-4 rounded-xl border shadow-sm overflow-x-auto">
        {pages.map((p) => (
          <button
            ...
            className={`py-3.5 px-4 text-xs font-bold border-b-2 transition whitespace-nowrap ${selectedRoute === p.route ? "border-black text-black" : "border-transparent text-gray-500 hover:text-black"
              }`}
          >
